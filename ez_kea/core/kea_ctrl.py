...
socket (Dhcp4/Dhcp6 -> control-socket -> socket-name) using Kea's JSON
command-channel protocol. Used for live status checks (e.g. "ha-heartbeat")
that a config-file edit can't answer -- those need a real response from the
running daemon, not the file on disk.
...
def send_command(socket_path: str, command: str, timeout: float = 5.0) -> Dict[str, Any]:
    ...
    if not socket_path:
        raise ControlChannelError("No control-socket is configured for this daemon")
